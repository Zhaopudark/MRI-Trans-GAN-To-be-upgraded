...
class c7s1_k_3D(tf.keras.Model):
    ...
    def __init__(self,filters,sn=False,use_bias=False,activation="relu",**kwargs):
        ...
        self.l1_conv3d = BaseLayers.Conv3D(filters=filters,kernel_size=[7,7,3],strides=[1,1,1],padding="REFLECT",use_bias=use_bias,sn=sn,**kwargs)
...
class dk_3D(tf.keras.Model):
    ...
    def __init__(self,filters,sn=False,use_bias=False,**kwargs):
        ...
        self.filters_zp = filters
        self.l1_conv3d = BaseLayers.Conv3D(filters=filters,kernel_size=[3,3,3],strides=[2,2,1],padding="REFLECT",use_bias=use_bias,sn=sn,**kwargs)
        self.l2_norm = BaseLayers.InstanceNorm3D()
        self.l3_activation = BaseLayers.ReLU()
    def build(self,input_shape):
        self.l1_conv3d.build(input_shape=input_shape)
...
class uk_3D(tf.keras.Model):
    ...
    def __init__(self,filters,sn=False,use_bias=False,**kwargs):
        ...
        self.filters_zp = filters
        self.l1_up= BaseLayers.UpSample3D(filters=filters,kernel_size=[3,3,3],strides=[2,2,1],padding="SAME",use_bias=use_bias,sn=sn,**kwargs)
        self.l2_norm = BaseLayers.InstanceNorm3D()
        self.l3_activation = BaseLayers.ReLU()
    def build(self,input_shape):
...
class ck_3D(tf.keras.Model):
    ...
    def __init__(self,filters,sn=False,norm=True,use_bias=False,**kwargs):
        ...
        self.filters_zp = filters
        self.l1_conv3d= BaseLayers.Conv3D(filters=filters,kernel_size=[4,4,3],strides=[2,2,1],padding='SAME',use_bias=use_bias,sn=sn,**kwargs)
        if norm:
            self.l2_norm = BaseLayers.InstanceNorm3D()
        else:
...
class last_conv_3D(tf.keras.Model):
    ...
    def __init__(self,use_sigmoid=True,sn=False,**kwargs):
        ...
        self.l1_conv3d= BaseLayers.Conv3D(filters=1,kernel_size=[4,4,3],strides=[1,1,1],padding='SAME',use_bias=True,sn=sn,**kwargs)
        if use_sigmoid:
