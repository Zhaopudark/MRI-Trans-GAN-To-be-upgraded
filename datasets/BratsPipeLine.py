...
class DataPipeLine():
    ...
    def get_centro_ranges(self):
        ranges_buf = []
        if self.dims == 3:
            ranges_buf.append([[56,183+1],[56,183+1],[76,78+1]])
        elif self.dims == 2:
            ranges_buf.append([[56,183+1],[56,183+1]])
        else:
            raise ValueError("Unsupported dims")
        return ranges_buf
    def get_fixed_ranges(self):
        """
        3D暂时不考虑
        ...
        ranges_buf = []
        if self.dims == 3:
            ranges_buf.append([[48,175+1],[22,149+1],[76,78+1]])
            ranges_buf.append([[48,175+1],[90,217+1],[76,78+1]])
            ranges_buf.append([[64,191+1],[22,149+1],[76,78+1]])
            ranges_buf.append([[64,191+1],[90,217+1],[76,78+1]])
        elif self.dims == 2:
    ...
    def generator(self):
        ...
        for i,(A,B,A_m0,B_m0) in enumerate(self.datalist):
            ...
            imgB_m0 = self.read_file(B_m0)
            if self.dims == 3:
                if self.crop=="crop_fixed":
                    ranges_buf = self.get_fixed_ranges()
                elif self.crop=="crop_random":
                    ranges_buf = self.get_random_ranges()
                elif self.crop=="crop_centro":
                    ranges_buf = self.get_centro_ranges()
                else:
                    raise ValueError("Unsupported crop ways"+self.crop)
...
if __name__ == "__main__":
    ...
    #         plt.figure(figsize=(5,5))#图片大一点才可以承载像素
    #         plt.subplot(2,2,1)
    #         plt.imshow(A[:,:,77],cmap='gray')
    #         plt.axis('off')
    #         plt.subplot(2,2,2)
    #         plt.imshow(A_m0[:,:,77],cmap='gray')
    #         plt.axis('off')
    #         plt.subplot(2,2,3)
    #         plt.imshow(B[:,:,77],cmap='gray')
    #         plt.axis('off')
    #         plt.subplot(2,2,4)
    #         plt.imshow(B_m0[:,:,77],cmap='gray')
    #         plt.axis('off')
    #         print(buf)
    #         plt.show()
    #     print(i)
